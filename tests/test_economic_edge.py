...
from fusion.economic_edge import apply_rotation_risk_caps, extended_pair_stop_prices, stable_rotation_target
...
def _rank_fixture():
    ...
    ind = {
        "atr_pct": pd.DataFrame(
            [{"a": 0.04, "b": 0.04, "c": 0.04}], index=idx
        ),
        "ret50": pd.DataFrame(
            [{"a": 0.50, "b": 0.40, "c": 0.39}], index=idx
        ),
        "ret80": pd.DataFrame(
            [{"a": 0.50, "b": 0.40, "c": 0.39}], index=idx
        ),
...
def test_small_rank_edge_keeps_rank3_incumbent():
    ...
    target = stable_rotation_target(
        rf, {"a": object(), "c": object()}, [], ind, day, cfg
    )
    assert target == ["a", "c"]
...
def test_disabled_hysteresis_preserves_legacy_raw_topn_semantics():
    day, rf, ind, cfg = _rank_fixture()
...
def test_extended_pair_stop_rejects_low_correlation_pair():
    ...
    assert extended_pair_stop_prices(
        positions, prev, open_row, low_row, prev_close, ind, 0.20, cfg
    ) == {}
...
def test_tail_loss_budget_only_scales_the_exposed_name():
    ...
    ret1 = pd.DataFrame(
        {"a": [0.01] * 19 + [-0.20], "b": [0.01] * 20}, index=dates
    )
    cfg = copy.deepcopy(CONFIG)
    cfg["rotation_contract"]["tail_risk"] = True
    cfg["rotation_contract"]["common_risk"] = False
    got = apply_rotation_risk_caps(
        {"a": 0.50, "b": 0.50}, 1.0, cfg, {"ret1": ret1}, dates[-1]
    )
