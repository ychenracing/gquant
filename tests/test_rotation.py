...
def _run(**ov):
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        return cfg, FusionEngine(cfg).run()
...
def test_sticky_no_cap_historical_diagnostic_is_reproducible():
    ...
    cfg.update({
        ...
        "rebalance_mode": "sticky", "entry_mode": "vote_gate", "winner_tilt": 2.5,
        "winner_tilt_factor": "volume", "max_positions": 6, "max_single_weight": 0.24,
        "winner_slot_mult": 1.25, "ramp_step": 0.40, "breadth_crash": 0.10,
        "channel_atr_mult": 1.5,
        "momentum_blend": {"ret5": 0.25, "ret14": 0.35, "ret63": 0.40},
        "chandelier_tiers": [
            {"profit": 0.08, "atr_mult": 3.5}, {"profit": 0.20, "atr_mult": 2.8},
            {"profit": 0.50, "atr_mult": 2.4}, {"profit": 1e9, "atr_mult": 2.1},
        ],
        "dd_abs_tiers": [{"dd": 0.10, "target": 0.70}, {"dd": 0.14, "target": 0.45}],
        "exposure_caps": {"TREND": 1.0, "WEAKEN": 0.85, "CRASH": 0.0, "RECOVERY": 1.0},
        "start": "2025-04-01", "end": "2026-07-24", "initial_capital": 1_000_000.0,
    })
    with contextlib.redirect_stdout(io.StringIO()):
...
def test_rotation_weights_equal_profile_is_v4_initial():
    from fusion.engine import _rotation_weights
    cfg = copy.deepcopy(CONFIG)
    ...
    got2 = _rotation_weights(
        ["a", "b", "c"], 0.9, dict(cfg, rotation_rank_weights=[1.0])
    )
...
def test_rotation_weights_underdeployment_is_capped_not_inflated():
    ...
    cfg["rotation_sizing"] = "equal"
    cfg["rotation_rank_weights"] = [1.0, 1.0]
    assert _rotation_weights(["a", "b"], 1.0, dict(cfg, max_single_weight=0.60)) == \
...
def test_atr_risk_budget_is_inverse_to_volatility():
    ...
    cfg = _base_atr_cfg(
        rotation_sizing="atr_risk_budget", rotation_risk_pct=0.04,
        rotation_atr_floor_pct=0.02, max_single_weight=0.50,
    )
...
def test_atr_risk_budget_does_not_scale_up():
    ...
    cfg = _base_atr_cfg(
        rotation_sizing="atr_risk_budget", rotation_risk_pct=0.03,
        rotation_atr_floor_pct=0.02, max_single_weight=0.50,
    )
    got = _rotation_weights(["a", "b"], 1.0, cfg, {"atr_pct": atr_pct}, idx[0])
...
def test_atr_risk_budget_uses_floor_for_missing_atr():
    ...
    cfg = _base_atr_cfg(
        rotation_sizing="atr_risk_budget", rotation_risk_pct=0.025,
        rotation_atr_floor_pct=0.02, max_single_weight=0.50,
    )
    got = _rotation_weights(["a", "b"], 1.0, cfg, {"atr_pct": atr_pct}, idx[0])
...
def test_vol_target_multiplier_disabled_and_bounded():
    from fusion.risk import vol_target_multiplier
